...
def parse_args():
    ...
    p.add_argument('--ray-length-km', type=float, default=4.0, help='Ray length in km (base, scaled by wind)')
    p.add_argument('--ray-curvature-km', type=float, default=1.2, help='Ray curvature amplitude in km')
    p.add_argument('--ray-angle-deg', type=float, default=20.0, help='Half-angle offset for side rays in three mode')
    p.add_argument('--ray-segments', type=int, default=12, help='Polyline segments per ray (smoothness)')
    ...
    p.add_argument('--ray-length-mult', type=float, default=1.0, help='Multiply computed ray length (after mode)')
    ...
    # Ray width scaling
    p.add_argument('--ray-width-scale', type=float, default=1.0, help='Scale factor for ray line widths')
    # Tail blinking dots
    p.add_argument('--tail-blink', action='store_true', help='Add blinking circles at ray tails')
    ...
    if not args.no_rays and args.rays > 0:
        ...
        for i, row in g_sorted.reset_index(drop=True).iterrows():
            ...
            if args.ray_length_mode == 'knn' and pd.notna(row.get('__ray_len_knn', np.nan)):
                ...
                # scale curvature with length to preserve shape proportion
                L_ref = max(1e-6, float(args.ray_length_km))
                C = args.ray_curvature_km * (L / L_ref)
            else:
                L = args.ray_length_km * wscale
                C = args.ray_curvature_km * wscale
            ...
            if args.rays_mode == 'three':
                ...
                for j, ang in enumerate(angles):
                    ...
                    if args.ray_emit:
                        ...
                        phase = (frame_idx / max(1, args.ray_emit_period) + base_phase + 0.33*j) % 1.0
                        ...
                        tail_lon.append(bx[tip_idx]); tail_lat.append(by[tip_idx])
                        # individual blinking phase per tail point
                        ...
                        tail_lon.append(lon2); tail_lat.append(lat2)
                        phi = 2*np.pi*rng.random()
                        alpha = np.clip(0.3 + 0.7*np.sin(float(args.tail_omega)*frame_idx + phi), 0.15, 1.0) * float(args.tail_opacity)
                        tail_color.append(_rgb_to_rgba_str(temp_to_rgb(tval, t_min, t_max), alpha))
            else:
                ...
                for k in range(offset, args.rays, step):
                    ...
                    if args.ray_emit:
                        base_phase = rng.random() * float(args.ray_emit_phase_jitter)
                        phase = (frame_idx / max(1, args.ray_emit_period) + base_phase) % 1.0
                        frac = max(args.ray_emit_min_frac, phase)
                        add_partial_segment(outer_lon_bins[b_idx], outer_lat_bins[b_idx], bx, by, frac)
                        add_partial_segment(mid_lon_bins[b_idx], mid_lat_bins[b_idx], bx, by, frac)
                        add_partial_segment(inner_lon_bins[b_idx], inner_lat_bins[b_idx], bx, by, frac)
